...
use_aloha = True  # Set to True to use ALOHA with ACT, False to use PushT with Diffusion
...
if use_aloha:
    ...
    output_directory = Path("outputs/eval/example_aloha_act_best_of_n_avg_reward")
    pretrained_policy_path = "lerobot/act_aloha_sim_insertion_human"
else:
    env_name = "gym_pusht/PushT-v0"
    output_directory = Path("outputs/eval/example_pusht_diffusion_best_of_n_avg_reward")
...
# Print policy configuration for debugging
...
rewards = []
max_rewards = []  # New tracker for max instantaneous rewards
avg_rewards = []
successes = []

# In this approach we rely on separate policy copies so we do not need an extra warmup step.
for episode in range(n_episodes):
    ...
    while not done:
        ...
        # (avg_reward, cumulative_reward, rollout_frames, rollout_action_buffer,
        ...
        for rollout in range(n_rollouts):
            ...
            steps_executed = 0
            finished_flag = False  # Indicates if the env terminated/truncated.
            success_flag = False  # Indicates if the termination was successful.
            max_instant_reward = -float("inf")  # Track maximum instantaneous reward
            ...
            rollout_results.append(
                (
                    avg_reward,
                    rollout_reward,
                    max_instant_reward,  # Add max instantaneous reward
                    rollout_frames,
                    rollout_action_buffer,
                    finished_flag,
                    success_flag,
        ...
        # Select the rollout with the highest average reward.
        best_rollout = max(rollout_results, key=lambda x: x[0])
        (
            best_avg_reward,
            best_segment_reward,
            best_max_reward,  # Extract max instantaneous reward
            best_segment_frames,
        ...
        segment_count += 1
        print(
            f"Episode {episode+1} - Segment {segment_count} complete: avg reward/timestep = {best_avg_reward:.2f}, "
            f"max reward/timestep = {best_max_reward:.2f}, "  # Add max reward to print statement
    ...
    max_rewards.append(best_max_reward)  # Store the max instantaneous reward
    avg_rewards.append(best_avg_reward)
