...
# from lerobot.common.policies.pretrained import load_pretrained_policy
...
def rollout(
    ...
    while not np.all(done):
        ...
        with torch.inference_mode():
            action = policy.select_actions(observation, n, calql)
        #=================================================================
...
def eval_main(cfg: EvalPipelineConfig):
    ...
    # print(q_list.keys())
    # for ep_idx, q_vals in q_list.items():
    # # Convert Q-values to CPU and scalars if needed
    #     q_vals_cpu = [q.item() if isinstance(q, torch.Tensor) else q for q in q_vals]
    #     plt.plot(q_vals_cpu, label=f"Episode {ep_idx}")
